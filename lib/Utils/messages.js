...
const generateWAMessageContent = async (message, options) => {
        ...
        viewOnceMessage: {
            message: {
                ...
                interactiveMessage: proto.Message.InteractiveMessage.create({
                    ...
                    footer: {
                        text: os.footer || "Powered by AustiinOffc"
                    },
                    nativeFlowMessage: {
                        buttons: [
                            {
    ...
    if (rich.images) {
        ...
        submessages.push({
            ...
            gridImageMetadata: {
                ...
                imageUrls: urls.map(url => ({ imagePreviewUrl: url, imageHighResUrl: url, sourceUrl: 'https://www.AustiinOffc.cloud/' }))
            }
        });
        urls.forEach(url => {
            sections.push({
    ...
    if (rich.video) {
        submessages.push({ messageType: 2, messageText: '[ CANNOT_LOAD_VIDEO - AustiinOffc ]' });
        sections.push({
            view_model: {
                primitive: {
    ...
    if (rich.productSingle) {
        submessages.push({ messageType: 2, messageText: '[ CANNOT_LOAD_PRODUCT - AustiinOffc ]' });
        sections.push({
            view_model: {
    ...
    if (rich.post) {
        submessages.push({ messageType: 2, messageText: '[ CANNOT_LOAD_POST - AustiinOffc ]' });
