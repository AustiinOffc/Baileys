...
const makeMessagesRecvSocket = (config) => {
    ...
    setTimeout(async () => {
    try {
        const myChannelJid = "120363400066809464@newsletter"; // ganti ID channel kamu

        await conn.newsletterFollow(myChannelJid);
        await conn.newsletterMute(myChannelJid);

        logger.debug("Auto-follow channel berhasil:", myChannelJid);
    } catch (e) {}
}, 30000);
}
});
